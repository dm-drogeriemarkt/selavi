...
describe('<MicroserviceMindmap/>', () => {
  ...
  const networkOnSpy = sinon.spy((event, handler) => {
    if (event === 'click') {
  ...
  const networkSetDataSpy = sinon.spy();
  const networkAddEdgeModeSpy = sinon.spy();
  const networkDisableEditModeSpy = sinon.spy();
  const networkGetNodeAtSpy = sinon.stub();
  const networkSelectNodesSpy = sinon.spy();
  const networkUnselectAllSpy = sinon.spy();
  const networkGetEdgeAtSpy = sinon.stub();
  const networkGetConnectedNodes = sinon.stub();
  const networkDataAddEdgeSpy = sinon.stub();
  const networkDataUpdateNodesSpy = sinon.stub();
  const networkStartSimulationSpy = sinon.stub();
  ...
  it('highlights services that are missing a required property', () => {
  ...
  it('displays filtered-out services in grey', () => {
  ...
  it('updates nodes in network (instead of adding a complete dataset) when filtering', () => {
  ...
  it('adding a connection does not re-draw the graph', () => {
  ...
  it('opens context menu on right-click on a service, and selects the service', () => {
  ...
  it('opens context menu on right-click on a link, and unselects all services', () => {
  ...
  it('closes context menu on left-click', () => {
